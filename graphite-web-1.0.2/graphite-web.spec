...
Patch0:         graphite-web-settings.patch
Patch1:         graphite-web-vhost.patch
Patch2:         graphite-web-wsgi.patch
...
%{__python} -c 'import setuptools; execfile("setup.py")' install --skip-build --prefix=%{_prefix} --install-data=%{_sysconfdir}/%{name} --install-lib=%{python_sitelib} --root %{buildroot}
